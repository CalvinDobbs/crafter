...
6. Prepare the wrists **while still at the top**: J5 yaw, blended with J6 so the hand stays level, creeps inward toward the box. `--hook` caps the travel; `--hook 0` skips the stage, as does `--lower-only`. Doing this before the descent means the hands arrive already turned in, rather than sweeping across the box at floor level.
7. J0 down to the calibrated bottom plus `--bottom-margin` turns back toward the top. **That margin is a lift offset, not a measured floor clearance** — nothing here senses the floor. `--lower-only` stops and holds at this pose.

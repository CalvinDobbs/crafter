...
try:
    ...
    # Optional laptop-generated script (push_voice.py → voice_script.json).
    _script_path = Path(__file__).resolve().parent / "voice_script.json"
    if _script_path.exists():
        import json as _json
        from flavor import load_script
        load_script(_json.loads(_script_path.read_text()))
        print(f"[mc_skills] loaded {_script_path.name}", flush=True)
...
def voice_script(r: ScriptReq):
    """Load laptop-generated lines into the narrator (no motion lock)."""
    try:
        from flavor import load_script
        load_script(r.lines)
        return {"ok": True, "n": len(r.lines)}
    except Exception as e:
        return {"ok": False, "error": f"{type(e).__name__}: {e}"}
